...
나도 이해가 부족하지만 직관적으로 보면 mean-variance 모델은 포트폴리오 수익률의 분산을 리스크라고 정의한다. 따라서 저 위의 회귀모형식은 $t =1,\dots,T$ 날의 포트폴리오 수익률을 1에 가장 가깝게 하는 그런 $\beta$를 찾는 문제를 의미한다. 단순하게 생각하면 모든 수익률이 1에 가까워지면 자연스럽게 수익률의 분산도 작아질 것이라고 생각한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ5Nzc1MTExOCwtMzUxMDU5MzIwLDE2Mz
