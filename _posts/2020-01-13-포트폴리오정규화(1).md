...
------------
### 3.1 Inverse problem 
$r_{t}, t=1,\cdots, T$는 자산 수익률들의 관측치 벡터이고 $R$은 $T \times N$ 행렬인데 $t$번째 행이 $r_{t}^{\top}$와 같다. $\Omega = E(r_{t}r_{t}') = E(R'R)/T$라고 하자. 여기서 관측치의 기대값 표현이 마음에 걸린다.
...
의 MSE(Mean Squared Error)를 가장 작게하는 회귀계수이다. 

<span style="color:#DAF7A6;">근데 이게 무슨 의미일까? <span>

>나도 이해가 부족하지만 직관적으로 보았을 때, mean-variance 모델은 포트폴리오 수익률의 분산을 리스크라고 정의한다. 따라서 저 위의 회귀모형식은 $t =1,\dots,T$ 날의 포트폴리오 수익률을 1에 가장 가깝게 하는 그런 $\beta$를 찾는 문제를 의미한다. 단순하게 생각하면 모든 수익률이 1에 가까워지면 자연스럽게 수익률의 분산도 작아질 것이라고 생각한다. 근데 유도를 위해서 1이 나왔지만 만약 기대수익률에 대한 제약조건$(\mu_0 = \hat{\mu}^{\top}\beta)$이 주어지면 $1_T$가 $\mu_0 *1_T$로 바뀌는지에 대해서는 Britten-Jones(1999) 논문에서 찾아보아야겠다. -> 읽어보았다[[포트폴리오와 회귀분석과의 관계]](https://chulhongsung.github.io/%EB%85%BC%EB%AC%B8%20%EC%9D%BD%EA%B8%B0/Britten-Jones/).
...
분자는 $1_T = R\beta + u$ 회귀분석의 OLS 추정치이고 분모는 1과 $\hat{\mu}^\top \hat{\beta}$의 차이(1과 평균 수익률의 차이)와 가중치 $\gamma$의 곱이다. 분모는 scalar이기 때문에 $\hat{\beta}$에 상수를 곱한 것이다. 이 의미에 대해서는 좀 더 생각해봐야겠다.
...
다음 포스팅에서 위의 문제들을 해결할 정규화 방법에 대해서 얘기한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTYyNTI3NTYxMiwxNzA1MTAwMDEwLDE1Nz
c1NjU4MDYsLTI1MzUyMjU5NywxNzg2NDAxNzE4LC0xMjU5MzI5
OTIyLC0xNjY2MzQ2MTIzLC05ODg5MjA5NjUsLTE2NjYzNDYxMj
MsLTQyMDk3MzE2NiwtMzUxMDU5MzIwLDE2MzIxODAxMTAsLTk4
ODQ3MDcyLDIwMTQ1MDI2MjQsLTEwOTY1MDcxMTIsMTYxOTM2MT
Y5OCwxMzQ2NjE3NDddfQ==
-->