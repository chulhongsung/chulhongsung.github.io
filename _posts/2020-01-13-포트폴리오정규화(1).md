...
위의 문제들은 $\hat{\Omega} = R^{\top}R/T$ 행렬에 크게 의존한다. 근데 자산들이 매우 큰 연관성이 있거나($\Sigma$가 _'nealy singular matrix'_ ) 혹은 자산의 수가 샘플 수보다 클 경우($\hat{\Sigma}$가 _'(nearly) singular matrix'_ ) 문제가 발생한다. 위와 같은 경우에 $\hat{\Omega}$는 0에 가까운 특잇값(singular value)을 갖는다. 그 이유는 $r(\hat{\Omega}) \leq min\{r(R^\top), r(R)\}$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA4MTcwOTIzNCw5OTcwNzYwMjksMzE0OD
