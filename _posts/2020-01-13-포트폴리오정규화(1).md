...
그 이유는 $r(R^\top R) \leq min\{r(R^\top), r(R)\}$이고 그러므로 $r(R^\top R) \leq T < N$가 된다. 최대계수행렬(full rank matrix)이 아니게 된다. 따라서, 최대계수행렬(full rank matrix)가 아니므로 역행렬이 존재하지 않는다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM1MzkzODg3NCw5OTcwNzYwMjksMzE0OD
