...
>나도 이해가 부족하지만 직관적으로 보았을 때, mean-variance 모델은 포트폴리오 수익률의 분산을 리스크라고 정의한다. 따라서 저 위의 회귀모형식은 $t =1,\dots,T$ 날의 포트폴리오 수익률을 1에 가장 가깝게 하는 그런 $\beta$를 찾는 문제를 의미한다. 단순하게 생각하면 모든 수익률이 1에 가까워지면 자연스럽게 수익률의 분산도 작아질 것이라고 생각한다. 근데 유도를 위해서 1이 나왔지만 만약 기대수익률에 대한 제약조건$(\mu_0 = \hat{\mu}^{\top}\beta)$이 주어지면 $1_T$가 $\mu_0 *1_T$로 바뀔것인가에 대해서는 한번 유도를 해봐야할 것 같다. Britten-Jones(1999) 논문을 더 읽어봐야겠다. 읽어보았다.[[포트폴리오와 회귀분석과의 관계]](https://chulhongsung.github.io/%EB%85%BC%EB%AC%B8%20%EC%9D%BD%EA%B8%B0/Britten-Jones/).
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMjY5OTE1NTYsLTI1MzUyMjU5NywxNz
