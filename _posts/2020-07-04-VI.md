...
근데 위의 $\mathbb{E}(z_2))$는 위의 유도과정에서 확인할 수 있지만 $q_2(z_2)$를 probability measure로 하는 기댓값이다. 따라서 우리는 알지 못한다. 하지만 위의 꼴을 보면 $q_1^{\star}(z_1)$ 역시 정규분포임을 알 수 있다.

$$
q_1^{\star}(z_1) = \mathcal{N}(z_1;\mu_1 -\Lambda_{11}^{-1}\Lambda_{12}(\mathbb{E}(z_2) - \mu_2), \Lambda_{11}^{-1})
$$ 

$q_2^\star$도 위와 같은 방식으로 똑같이 구할 수 있고 정규분포임을 알 수 있다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNTgzNTA3NDQsMTQ5NzM5NjY0OCw1Mj
g0OTk1NTYsLTEyNTc4Mzg5MzMsLTIwMTQ4MTkxNiw4NDU4ODgy
NTMsMTczMjY0NzQ3MiwzMzEwMzA1MjEsLTY0MzUxMTcyOCwxMz
