...
위의 식에서 앞 항, $\mathcal{L}(q)$을 많은 책에서 ELBO(Evidence Lower Bound) 혹은 Variational Free Energy라고 한다. 뒤의 항은 분포간의 유사도(?)의 측도로 쓰일 수 있는 KL-divergence이다.  위의 식에서 $\log p({\bf X})$가 고정이고 ELBO를 최대화하는 것은 KL-divergence를 최소화하는 것과 동일해진다. 이 흐름은 처음의 추론의 목적과 연결된다. KL-divergence가 줄어든다는 것은 사후확률분포와 $q({\bf Z})$이 유사해지는 것이다. 그럼 구할 수 업 이처럼 $q({\bf Z})$를 도입해서 ELBO를 최대화함으로써 계산불가능한 KL-divergence를 간접적으로 줄인다. 여기서, $q({\bf Z})$를 variational distribution이라고 한다.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU1MjUxMDQ0MCwxMzk5MjUxMzE2LC0xMD
