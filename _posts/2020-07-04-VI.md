...
위와 같은 분포를 *factorized distributio* 이라고 한다.
...
$$
\begin{aligned}
\ln q^{\star}_1(z_1) &= \mathbb{E}_{z_2}[\ln p( {\bf Z})] + \text{const}\\
&= \mathbb{E}_{z_2}[-\frac{\Lambda_{11}}{2}(z_1 - \mu_1)^2 - \Lambda_{12}(z_1 - \mu_1)(z_2 - \mu_2)] + \text{const} \\
&=  -\frac{\Lambda_{11}}{2}z_1^2 + (\mu_1\Lambda_{11} - \Lambda_{12}(\mathbb{E}(z_2) - \mu_2))z_1 + \text{const}.
\end{aligned}
$$

근데 위의 $\mathbb{E}(z_2))$는 위의 유도과정에서 확인할 수 있지만 $q_2(z_2)$를 probability measure로 하는 기댓값이다. 위의 꼴을 보면 $q_1^{\star}(z_1)$ 역시 정규분포임을 알 수 있다.

$$
q_1^{\star}(z_1) = \mathcal{N}(z_1;\mu_1 -\Lambda_{11}^{-1}\Lambda_{12}(\mathbb{E}(z_2) - \mu_2), \Lambda_{11}^{-1})
$$ 

$q_2^\star$도 위와 같은 방식으로 똑같이 구할 수 있고 정규분포임을 알 수 있다. 초기값을 설정하고 반복적으로 $q_1^\star$와 $q_2^\star$를 수렴할 때까지 구한다면 해를 찾을 수 있을 것이다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTQ1NTc4NDYyLC0xMjU4MzUwNzQ0LDE0OT
czOTY2NDgsNTI4NDk5NTU2LC0xMjU3ODM4OTMzLC0yMDE0ODE5
MTYsODQ1ODg4MjUzLDE3MzI2NDc0NzIsMzMxMDMwNTIxLC02ND
M1MTE3MjgsMTM5OTI1MTMxNiwtMTA5MDM0NDY0NywxNDYyMTM1
MzMsLTgzOTU3NjQ4OCwtMjA4NjYwNDI0NywxNDkzODgwMDk2LD
UwMzM0ODIzOSwxOTE5MjE4ODEzLDE3OTMzOTM0NTEsNjE0MDA0
MzA0XX0=
-->