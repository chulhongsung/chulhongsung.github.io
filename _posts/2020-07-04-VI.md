...
Variational inference는 위에서 설명한 factorized approximation을 기반인데 이 방법도 문제가 있다. 그 문제를 이제 예를 통해서 확인해보자. 만약 실제 문제에서는 우리가 알 수 없지만 실험에서 $p({\bf X}, {\bf Z})$를 알고 있다고 하자. 이 실험에서는 데이터 $\bf X$가 없으므로 그냥 $p({\bf Z})$라고 하겠다. $p({\bf Z}) = \mathcal{N}({\bf Z} ; {\bf \mu}, {\bf \Lambda}^{-1})$이고 여기서 가우시안 분포의 모수들은 각각 아래와 같다.
...
위에서 처럼 $q({\bf Z}) = q_1(Z_1)q_2(Z_2)$로 생각하자.

<!--stackedit_data:
eyJoaXN0b3J5IjpbNTM5NTgzMjU2LC0xMjU3ODM4OTMzLC0yMD
E0ODE5MTYsODQ1ODg4MjUzLDE3MzI2NDc0NzIsMzMxMDMwNTIx
LC02NDM1MTE3MjgsMTM5OTI1MTMxNiwtMTA5MDM0NDY0NywxND
