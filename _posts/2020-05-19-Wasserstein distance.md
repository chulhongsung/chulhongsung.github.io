...
이번에 RBM이 필요로 한 문제가 발생하여 다시 책을 폈다. 내용은 Goodfellow의 'Deep learning' 책과 전북대 오일석 교수님의 '기계 학습' 책을 참고했다. 그리고 수식의 전개와 표기법은 서울시립대 최호식 교수님의 수업 자료를 참고했다.
## <span style="color:#00ADB5;"> Wasserstein Restricted Boltzmann Machines(WRBM)</span>
...
RBM에서는 단순하다. 데이터 $\bf x$ 를 이용해서 $\bf h$를 계산하고, $\bf h$를 가지고 반대로 $\tilde{\bf x}$ 생성한다.  수식으로 정리하면 아래와 같다. 아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자. 

$$
\begin{aligned}
p({\bf h}^{(n)} = 1  | {\bf x}^{(n)}) &= sigmoid({\bf W}{\bf x}^{(n)} + {\bf c})\\
p(\tilde{\bf x}^{(n)} = 1 | {\bf h}^{(n)}) &= sigmoid({\bf W}^\top {\bf h}^{(n)} + {\bf b})\\
\end{aligned}
$$

위의 식은 이항변수라는 특징 덕분에 위와 같이 식이 쓰여지는 것을 확인할 수 있다. 확률을 계산했으면 0과 1을 가지는 이항변수를 생성하는 것은 균일분포를 통해 쉽게 생성할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTkxMzQwNzAwMF19
-->