...
### <span style="color:#00ADB5;"> 1. $\textsf{pd.read\_sql(query ,conn)}$을 통해 테이블을 부르는 중 'Lost Connection' 에러가 발생한다. </span>
-----------
...
### <span style="color:#00ADB5;"> 2. $\textsf{pd.to\_sql(table\_name, conn)}$으로는 $\textsf{DataFrame}$이 DB에 올라가지지 않는다.</span>
...
여기까지가 제가 정리한 오류와 해결법이었습니다. 추가로 프로젝트 중 전처리를 하면서 느낀 것은 $\textsf{R}$이 확실히 익숙한 언어라서 그런 것일 수 있지만 확실히 $\textsf{`dplyr'}$ 패키지는 $\textsf{pandas}$ 패키지보다 매우 간편하고 강력하다는 생각이 들었습니다. 특히, pipe operator(%>%)와의 연계는 다른 패키지의 추종을 불허하는 것 같습니다. $\textsf{pandas}$도 더 공부하다보면 생각이 달리질지 궁금하긴합니다. 

이번 글에서는 $\textsf{pandas}$와 $\textsf{pymysql}$로 작업하던 중에 발생한 에러 대해서 얘기하고 나름의 해결책을 적어봤습니다. 평소에 데이터를 'csv'만 다루다가 막상 DB를 손대니 고생하면서 느낀 경험담이었습니다. 아무래도 처음하다보니 효율성은 매우 떨어질테고 궁금한 것도 많습니다. 예를들면, conn.close()를 매번 해주고 다시 conn을 선언해주어야 하는 것인가? 와 같이 DB에 대한 지식들입니다. 잘 아시는 분들이 의견을 주시면 감사하겠습니다. 긴 글을 읽어주셔서 감사합니다. 


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyODE0ODEzOTEsLTExNjc2NDkxNywtMj
AyMDg5NjAxMCwzMjc5ODg3MzAsLTI4MTE3ODYxNCwtMjgxMTc4
NjE0LDQ5NjIyNTgzMiwtMzgyOTkzNDE1LDQ1NjM1OTQ2NiwtMT
EzMjU0MjgxLC0xMzUzMTgwNDEzLDEwMDUzMDE4MjksMjE0NjU1
ODc5MSw5NTE2ODI5OTcsMTMwMzc5ODYzMiwtMTkzMjM1OTgzNC
wtMzQ4OTc3Nzk4LC00MzE2NjA0MTldfQ==
-->