...
최근 진행된 프로젝트에서 $\textsf{python}$으로 작업을 했습니다. 주로 $\textsf{R}$을 사용하고 $\textsf{python}$을 집중적으로 다루어본 것은 처음이었습니다. 이번 프로젝트는 DB에 직접 데이터 프레임을 올리는 작업까지 포함하고 있어서 DB를 다룰 기회가 없었던 저에게는 정말 좋은 기회였습니다. 
...
-----------
$\textsf{HeidiSQL}$을 통해 DB에 어떤 테이블이 있는지 확인을 했으면 테이블을 전처리 작업을 해야하는 $\textsf{python}$ 환경으로 불러와야합니다. $\textsf{pandas}$의 함수 $\textsf{`pd.read\_sql(query ,conn)'}$을 통해 테이블을 $\textsf{pandas DataFrame}$ 객체로 바로 받을 수 있습니다. 하지만 만약 내가 받아야할 테이블이 너무 커서 DB의 쿼리 유지시간을 초과하는 경우 지긋지긋한 '( 2013 - Lost Connection to MySQL server during query )'가 발생할 가능성이 있습니다. 근본적인 해결방법은 DB의 'wait_timeout' 옵션을 늘리거나(Stack Overflow에서 본 해결책) 더 좋은 DB 서버를 사용하는 것(실제로 프로젝트 중 DB 서버를 좋은 것으로 교체한 후 한 번도 에러가 발생하지 않았습니다)이 있습니다. 하지만 저는 당장 받아서 써야하는 상황이라 다음과 같이 해결했습니다.
```python
...
## <span style="color:#00ADB5;"> 3. $\textsf{argparse}$ 패키지를 이용해서 쿼리 변경하기</span>

추가로 한 작업은 터미널창에서 $\textsf{python}$ 스크립트를 실행시키면 테이블의 특정 행만을 뽑아오는 작업이었습니다. 그래서 $\textsf{argparse}$ 패키지를 사용해서 argument를 주었을 경우에 스크립트 내에서 문자열로 쿼리와 결합하도록 코드를 짜야했습니다. $\textsf{argparse}$ 패키지를 처음 사용하다보니 모르는 것이 많았는데 [Stack Overflow][https://stackoverflow.com/questions/15753701/how-can-i-pass-a-list-as-a-command-line-argument-with-argparse](https://stackoverflow.com/questions/15753701/how-can-i-pass-a-list-as-a-command-line-argument-with-argparse)에서 아주 좋은 예시를 보여줘서 쉽게 할 수 있었습니다.
...
이번 글에서는 $\textsf{pandas}$와 $\textsf{pymysql}$로 작업하던 중에 발생한 에러 대해서 얘기하고 나름의 해결책을 적어봤습니다. 평소에 데이터를 'csv'만 다루다가 막상 DB를 다루어보면서 고생한 경험담이었습니다. 아무래도 처음하다보니 효율성은 매우 떨어질테고 궁금한 것도 많습니다. 예를들면, conn.close()를 매번 해주고 다시 conn을 선언해주어야 하는 것인가? 와 같이 DB에 대한 지식들입니다. 잘 아시는 분들이 의견을 주시면 감사하겠습니다. 긴 글을 읽어주셔서 감사합니다. 


<!--stackedit_data:
eyJoaXN0b3J5IjpbODc3MTM2ODc1LC05ODQ2Njk5NDQsNTY3MD
A0MDAzLDE2OTU0ODg0NTgsLTExNjc2NDkxNywtMjAyMDg5NjAx
MCwzMjc5ODg3MzAsLTI4MTE3ODYxNCwtMjgxMTc4NjE0LDQ5Nj
