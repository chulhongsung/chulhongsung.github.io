...
>$$
>
>위의 식을 보면 가능도함수는 모수(parameter), 여기서는 $\theta$의 함수이다. 따라서 우리는 관측된 데이터의 가능도를 가장 크게 해주는 모수$(\theta)$를 구하고 싶어하는 것이다.  가능성이 가장 높기 때문에 관측됐을 거라는 믿는 것이다. 예를 들면, 동전을 10000번 던졌는데 앞면이 7000번 나오면 앞면이 나올 확률($\theta$)를 0.7로 생각할 것이다. 이 방법을 "최대가능도추정법(Maximum Likelihood estimation)", 줄여서 MLE라고 한다.  

위의 가능도함수는 일반적으로 확률밀도함수(PDF)의 곱으로 나타내는데 $log$를 취하면 합으로 표현되기 때문에 로그가능도함수를 많이 사용한다. 중요한 것은 가능도를 최대화하는 것이다. 근데 일반적으로 기계학습 분야에서는 최소화문제로 최적화를 한다. 따라서 음의 로그가능도함수(negative log likelihood)를 최소화하는 문제로 생각한다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMzM5MzA4OTU4LC0yMDM3MTU1NTk3LDc5Mj
