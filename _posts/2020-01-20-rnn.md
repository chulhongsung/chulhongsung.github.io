...
먼저 왼쪽 그림을 보면 입력값 $x$가 모델로 들어가서 연산을 통해 은닉 상태 $h$가 되는데 $h$는 다시 모델로 들어가도록 설계가 되어있는 것을 확인할 수 있다. 개인적으로 오른쪽에 있는 "Unfold 그림"을 더 선호하는데 그 이유가 모델을 펼쳐보면 더 이해가 잘 된다. $x^{(t)}$는 여기서 $t$ 시점의 입력값이다. $t$시점의 입력값 $x^{(t)}$이 신경망 모델로 들어가게 되고 연산이 되어서 나온ㅇ $h^{(t)}$
...
-----------
<!--stackedit_data:
eyJoaXN0b3J5IjpbNDE2NDk1NzM5XX0=
-->