...
학습을 위해서는 한 단계를 더 거쳐야한다. 바로 "softmax" 단계이다. 이 과정을 통해 위에서 언급한 정규화(normalized)가 되고 더 확률의 개념과 가까워진다. softmax는 다음과 같다.

$$
softmax(o^{(t)}) = \frac{exp(\textbf{o}^{(t)})}{}
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTI4MzA2NTM4LC0xODUwODcwMzE3LC0zOT
