...
> 위의 단어는 Ian Goodfellow가 쓴 "Deep Learning" 책의 내용을 인용한 것이다. $\bf{o}^{(t)}$을 로그확률로 생각하자고 했지만 모델 어디에도 $\bf{o}^{(t)} \leq 0_{n\times1}$에 대한 제약조건은 없다. 로그확률값은 확률값과 로그값이 동시에 저 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg2MzEyMDE4OSwtMTg1MDg3MDMxNywtMz
