...
>$$
>
>위의 식을 보면 가능도함수는 모수(parameter), 여기서는 $\theta$의 함수이다. 따라서 우리는 관측된 데이터의 가능도를 가장 크게 해주는 모수$(\theta)$를 구하고 싶어하는 것이다. 우리는 관측된 데이터를 가장 신뢰하기 때문이다. 예를 들면, 동전을 10000번 던졌는데 앞면이 7000번 나오면 앞면이 나올 확률($\theta$)를 0.7로 생각할 것이다. 이 방법을 "최대가능도추정법(Maximum Likelihood estimation)", 줄여서 MLE라고 한다.  

위의 가능도함수는 일반적으로 확률밀도함수(PDF)의 곱으로 나타내는데 $log$를 취하면 합으로 표현되기 때문에  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ4MzIyNTIwOCwtMjAzNzE1NTU5Nyw3OT
