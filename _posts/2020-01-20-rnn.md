...
먼저 왼쪽 그림을 보면 입력값 $x$가 모델로 들어가서 연산을 통해 $h$가 되는데 $h$는 다시 모델로 들어가도록 설계가 되어있는 것을 확인할 수 있다. 개인적으로 오른쪽에 있는 "Unfold 그림"을 더 선호한다. 모델을 펼쳐보면 더 이해가 잘 된다. $x^{(t)}$는 여기서 $t$ 시점의 입력값이다. 시점에 따라 다른 입력값들이 신경망 모델로 
...
-----------
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTY4NDEyMzg4NF19
-->