...
>가능도함수는 말 그래도 확률변수들의 가능도를 나타내는 함수이다. 그럼 가능도랑 확률이랑 다른 것인가? 확률은 $[0,1]$의 값을 갖지만 가능도는 그렇지 않아도 된다. 확률변수$(\textbf{X})$가 데이터 $$ 우리가 갖고 있는 "데이터에 대한 가능도(성)"를 나타내는 것이다. 즉, 클수록 가능성도 큰 것이다. $n$개의 확률변수$\textbf{X}=(X_1, \dots, X_n)$가 $\textbf{x} =(x_1, \dots, x_n)$(데이터)로 관측됐을 경우, 가능도함수 $L$은 다음과 같이 정의한다. 
>
>$$ 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNzgxNjkzMDQzLDgyOTk1NDk3MSwxODgwOT
