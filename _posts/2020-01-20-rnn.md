...
먼저 왼쪽 그림을 보면 입력값 $\bf{x}$가 모델로 들어가서 계산을 통해 은닉 상태 $\bf{h}$가 되는데 $\bf{h}$는 다시 모델로 들어가도록 설계가 되어있는 것을 확인할 수 있다. 개인적으로 오른쪽에 있는 "Unfold 그림"을 더 선호하는데 그 이유가 모델을 펼쳐보면 더 이해가 잘 된다.
...
### 2.2 RNN 구조(structure)와 손실함수(loss function)

먼저 구조와 $t$ 시점에 RNN 안에서 일어나는 계산을 알아보자.
...
-----------
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjUzOTQ0NDg2LC0xOTAyODcyODkyLC01OT
