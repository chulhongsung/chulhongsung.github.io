...
>가능도함수는 말그래도 가능도를 나타내는 함수이다. 그럼 가능도랑 확률이랑 다른 것인가? 확률은 $[0,1]$의 값을 갖지만 가능도는 그렇지 않다. 가능도는 실수값을 그냥 가진다. 우리가 갖고 있는 데이터에 대한 가능도를 나타내는 것이다. $n$개의 확률변수 집합데이터 $\textbf{X}=(X_1, \dots, X_n)$가 $\textbf{x} =(x_1, \dots, x_n)$로 관측됐을 경우, 가능도함수 $L$은 다음과 같이 정의한다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjEyNDY3MTMwNyw3OTIyNzcxMTUsMjAyNz
