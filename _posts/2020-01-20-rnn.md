---
title: "RNN(Recurrent Neural Network) (1)"
...
추가로 자연어 처리(NLP) 분야에서는 출력물 $\bf{o}^{(t)}$을 정규화되지 않은 로그확률(unnormalized log probability)로 생각하는 경우가 많다. 출력물 $\bf{o}^{(t)}$이 $n$차원의 벡터$(\bf{o}^{(t)} \in \mathbb{R}^n)$이면 $\bf{o}^{(t)}$의 각 원소(element)들은 원소와 대응하는 단어가 발생할 로그확률값이 된다.
...
이제 단어의 발생확률이 출력값이니 손실함수도 새롭게 정의해보자. 출력값이 확률인 경우에는 다양한 손실함수를 정의할 수 있지만(cross-entropy같은) 통계학에서 사용하는 가능도함수(likelihood function)를 생각할 수 있다. 가능도함수(likelihood function)는 통계학에서 가장 중요한 개념 중에 하나이다(추정, 검정 등). 기억이 맞다면 통계학의 아버지라고 불리는 R.A. Fisher가 제안했다는 걸로 알다. 가능도는 확률과 다르다. 기회가 된다면 더 자세하게 글을 쓰고 싶지만 여기서는 통계 비전공자를 위해 간략하게 소개하겠다.

<span style="color:#DAF7A6;">가능도함수(likelihood function)</span>
>가능도함수는 말그래도 가능도를 나타내는 함수이다. 그럼 가능도랑 확률이랑 다른 것인가? 확률은 $[0,1]$의 값을 갖지만 가능도는 그렇지 않다. 가능도는 실수값을 그냥 가진다. 우리가 갖고 있는 "데이터에 대한 가능도(성)"를 나타내는 것이다. 즉, 클수록 가능성도 큰 것이다. $n$개의 확률변수$\textbf{X}=(X_1, \dots, X_n)$가 $\textbf{x} =(x_1, \dots, x_n)$(데이터)로 관측됐을 경우, 가능도함수 $L$은 다음과 같이 정의한다. 
>
>$$ 
>L(\theta;\textbf{x}) = Pr(\textbf{X}=\textbf{x};\theta)
>$$
>
>위의 식을 보면 가능도함수는 모수(parameter), 여기서는 $\theta$의 함수이다. 따라서 우리는 관측된 데이터의 가능도를 가장 크게 해주는 모수$(\theta)$를 구하고 싶어하는 것이다.  가능성이 가장 높기 때문에 관측됐을 거라는 믿는 것이다. 예를 들면, 동전을 10000번 던졌는데 앞면이 7000번 나오면 앞면이 나올 확률($\theta$)를 0.7로 생각할 것이다. 이 방법을 __"최대가능도추정법(Maximum Likelihood estimation)"__, 줄여서 __"MLE"__ 라고 한다.  

위의 가능도함수는 일반적으로 확률밀도함수(PDF)들의 곱으로 나타내는데 $log$를 취하면 확률밀도함수들의 합으로 표현되기 때문에 로그가능도함수를 많이 사용한다. 중요한 것은 가능도를 최대화하는 것인데 $log$도 단조증가하는 함수이기 때문에 일관성은 유지된다. 근데 일반적으로 기계학습 분야에서는 최소화 문제로 최적화를 한다. 따라서 음의 로그가능도함수(negative log likelihood)를 최소화한다. 결국 가능도를 최대화하는 것이다.

가능도에 대한 얘기가 길었다. 다시 돌아와서 RNN 모델의 손실함수를 음의 로그가능도함수로 다음과 같이 정의할 수 있다.

$$
\sum_t L^{(t)} = -\sum_t logp(y^{(t)}, \textbf{x}^{(1)},\dots,\textbf{x}^{(t)} ; \textbf{W}, \textbf{U}, \textbf{V}, \textbf{b}, \textbf{c})
$$

여기서, $y^{(t)}$는 실제 정답에 대응되는 $\hat{y}^{(t)}_i$이다.

결국 손실함수는 가중치 행렬$(\textbf{W}, \textbf{U}, \textbf{V})$들과 절편 벡터$(\textbf{b}, \textbf{c})$, 즉 모수들에 대한 식으로 쓰여진다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1OTgxNjQxMTYsMTg4MDk5OTEyOSwtMT
k4MDEzNjM4NSwxMDU5OTg0NDgwLC0yMDM3MTU1NTk3LDc5MjI3
NzExNSwyMDI3NzkxODY3LC0xODUwODcwMzE3LC0zOTcxNTQ4MT
ksLTEwNTA1OTc0OTIsMTYxNDY2NTk1MCwtNzc2NjAxODgyLDI3
NjAzODExMiwxODg4NDAwNTM1LDE5NDI0OTE5MzAsMjA2MjI5ND
Q0LDc3NTUwMDI4MCwxNDQxNzE5ODI4LC0xOTAyODcyODkyLC01
OTAwMTQ0MTZdfQ==
-->