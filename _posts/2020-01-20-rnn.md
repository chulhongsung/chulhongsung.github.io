...
출력값이 이해되기 쉽고 확률처럼 와닿게 하기 위해서는 단계가 하나 더 필요하다. 바로 "softmax" 단계이다. softmax를 통해 출력값들의 합은 $1$이 된다. $t$ 시점에서 $i$번째 단어의 로그확률값$(o^{(t)}_i)$은 softmax를 거치면 $\hat{y}^{(t)}_i \in [0,1]$을 값을 갖는다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2MzkxNjM1MDMsODI5OTU0OTcxLDE4OD
