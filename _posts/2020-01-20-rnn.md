...
자연어 처리(NLP) 분야에서는 출력물 $\bf{o}^{(t)}$을 정규화되지 않은 로그확률(unnormalized log probability)로 생각하는 경우가 많다. 출력물 $\bf{o}^{(t)}$이 $n$차원의 벡터$(\bf{o}^{(t)} \in \mathbb{R}^n)$이면 $\bf{o}^{(t)}$의 각 원소(element)들은 원소와 대응하는 단어가 발생할 로그확률값이 된다.
...
출력값이 더 확률처럼 와닿게 하기 위해서는 단계가 하나 더 필요하다. 바로 "softmax" 단계이다. 이 과정을 통해 위에서 언급한 정규화(normalized)가 되고 더 확률의 개념과 가까워진다. $i$번째 단어의 로그확률값을 softmax를 통해서 $[0,1]$을 값을 갖는다.
...
결국 손실함수는 가중치 행렬$(\textbf{W}, \textbf{U}, \textbf{V})$들과 절편 벡터$(\textbf{b}, \textbf{c})$, 즉 모수들에 대한 식으로 쓰여진다. 그리고 정답이 나올 가능성을 가장 크게 해주는 가중치행렬과 절편 벡터들을 찾는 것이다. 최대가능도추정법과 연결되는 것이다.


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMzgxNTI1NDAsODI5OTU0OTcxLDE4OD
