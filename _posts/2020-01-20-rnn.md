...
### 2.3 RNN in NLP(Natural Language Process)

자연어 처리 분야에서는 출력물 $\bf{o}^{(t)}$을 정규화되지 않은 로그확률(unnormalized log probability)로 생각할 수 있다. 출력물 $\bf{o}^{(t)}$이 $n$차원의 벡터$(\bf{o}^{(t)} \in \mathbb{R}^n)$이면 $\bf{o}^{(t)}$의 각 원소(element)들은 원소와 대응하는 단어가 발생할 로그확률값이 된다.
...
결국 손실함수는 가중치 행렬$(\textbf{W}, \textbf{U}, \textbf{V})$들과 절편 벡터$(\textbf{b}, \textbf{c})$, 즉 모수들에 대한 식으로 쓰여진다. 그리고 정답 단어가 나올 가능성을 가장 크게 해주는 가중치행렬과 절편 벡터들을 찾는 것이다. 최대가능도추정법과 연결되는 것이다.


<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAwMDk2NTc5LC0xNjYyMDQ4NDMzLDgyOT
k1NDk3MSwxODgwOTk5MTI5LC0xOTgwMTM2Mzg1LDEwNTk5ODQ0
ODAsLTIwMzcxNTU1OTcsNzkyMjc3MTE1LDIwMjc3OTE4NjcsLT
E4NTA4NzAzMTcsLTM5NzE1NDgxOSwtMTA1MDU5NzQ5MiwxNjE0
NjY1OTUwLC03NzY2MDE4ODIsMjc2MDM4MTEyLDE4ODg0MDA1Mz
UsMTk0MjQ5MTkzMCwyMDYyMjk0NDQsNzc1NTAwMjgwLDE0NDE3
MTk4MjhdfQ==
-->