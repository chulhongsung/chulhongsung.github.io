...
>$$
>
>위의 식을 보면 가능도함수는 모수(parameter), 여기서는 $\theta$의 함수이다. 따라서 우리는 관측된 데이터의 가능도를 가장 크게 해주는 모수$(\theta)$를 구하고 싶어하는 것이다. 예를 들면, 동전을 100번 던졌는데 앞면이 70번 나오면 우리는 앞면이 나올 확률, 베르누이분포로 따지면 B(p).이 방법을 "최대가능도추정법(Maximum Likelihood estimation)", 줄여서 MLE라고 한다.  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEwMzgxNDkxOTAsLTIwMzcxNTU1OTcsNz
