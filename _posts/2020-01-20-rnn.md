...
이제 단어의 발생확률이 출력값이 되었으니 그에 걸맞는 손실함수도 새롭게 정의해보자. 출력값이 확률인 경우에는 다양한 손실함수를 정의할 수 있지만 통계학에서 사용하는 가능도함수(likelihood function)도 고려할 수 있다. 가능도함수는 통계학에서 가장 중요한 개념 중에 하나이다(추정, 검정 등). 기억이 맞다면 "통계학의 아버지"라고 불리는 R.A. Fisher가 제안했다. 가능도는 확률과 다르다. 기회가 된다면 더 자세하게 글을 쓰고 싶지만 여기서는 통계 비전공자를 위해 간략하게 소개하겠다.

<span style="color:#DAF7A6;">가능도함수(likelihood function)</span>
>가능도함수는 말 그래도 확률변수들의 가능도를 나타내는 함수이다. 그럼 가능도랑 확률이랑 다른 것인가? 확률은 $[0,1]$의 값을 갖지만 가능도는 그렇지 않아도 된다. 
>확률변수$(\textbf{X})$가 데이터 $(\textbf{x})$로 관측된었다면, $\textbf{x}$의 가능도함수는 "$\textbf{x}$에 대한 가능도(성)"를 나타내는 것이다. 즉, 클수록 가능성도 큰 것이다. $n$개의 확률변수$\textbf{X}=(X_1, \dots, X_n)$가 $\textbf{x} =(x_1, \dots, x_n)$(데이터)로 관측됐을 경우, 가능도함수 $L$은 다음과 같이 정의한다. 
>
>$$ 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzMxNjM2OTEsODI5OTU0OTcxLDE4OD
A5OTkxMjksLTE5ODAxMzYzODUsMTA1OTk4NDQ4MCwtMjAzNzE1
NTU5Nyw3OTIyNzcxMTUsMjAyNzc5MTg2NywtMTg1MDg3MDMxNy
wtMzk3MTU0ODE5LC0xMDUwNTk3NDkyLDE2MTQ2NjU5NTAsLTc3
NjYwMTg4MiwyNzYwMzgxMTIsMTg4ODQwMDUzNSwxOTQyNDkxOT
MwLDIwNjIyOTQ0NCw3NzU1MDAyODAsMTQ0MTcxOTgyOCwtMTkw
Mjg3Mjg5Ml19
-->