...
하지만 행렬 $R_{T \times N}$에서 $T < N$이면  $R^\top R$은 비정칙행렬(singular matrix)이다. 즉, 역행렬을 구할 수 없다. 그 이유는 

$R^\top R$은 $N \times N$차원의 정방행렬인데 $R^\top R$의 계수(rank)는 $T$보다 작거나 같다. 따라서, 최대계수행렬(full rank matrix)가 아니므로 역행렬이
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMjY4NTk3OTgsLTE5MDg4NTg1MjYsMz
