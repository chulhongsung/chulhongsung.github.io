...
\frac{\partial \log p_{\theta}({\bf x})}{\partial c_k} &= \frac{1}{n}\sum_{i=1}^n p(h_k^{(i)}= 1|{\bf x}^{(i)}) -\sum_{\tilde{\bf x}}p_{\theta}p(h_k = 1|{\bf x}), \\
\frac{\partial F_{\theta}({\bf x})}{\partial w_{jk}} &= -x_jp(h_k = 1 | {\bf x}).
\end{aligned}
$$


그리고 $p_{emp}$를 현재 데이터에 대한 경험 분포($\sum_{i=1}^n \delta_{{\bf x}^{(i)}}$/n)이라 하자. 

<!--stackedit_data:
eyJoaXN0b3J5IjpbNDc1MjYwMjk3LDc1MDkyMTYxMSw2NjEwMj
E3MTYsNzMwOTc3MjYzLC0xNDI4ODg4OTMyLC04MTc3NjUwMjMs
MTE0MDI5MTg0MywxMTQwMjkxODQzLC0xMTM0NzM5ODYxLDY3Mz
YzMzY4MywxMzQ4MDEwNjU3LDEwNjA5NzI2NTYsMjA1NDM0Mzgz
MiwxMzE1MzI0NDU3LDEyMjkxNDQ4MzYsMjAyODgzMjczMiw1MD
U0MzE5OTMsLTIwMTY3Nzk3NDMsMjAzMDU4NTIxLDc3NzQ4OTY3
NF19
-->