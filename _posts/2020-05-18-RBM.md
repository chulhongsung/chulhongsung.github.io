...
RBM의 목적함수는 marginal likelihood $p_{\theta}({\bf x})$를 최대화하는 것이다. 즉, 우리가 가지고 있는 데이터(x)의 likelihood를 높이는 것이다. log-likelihood, $\log p_{\theta}({\bf x})$를 모형의 파라미터($\theta$)에 대해서 미분해보자.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjkxNTY3MDYzLDEyNzUzOTA1NTksLTI1NT
U1NDk1MSwxNjMwMjUzNDk0LC01Mjg1MDAxNTYsLTMxNDkxNzI1
NF19
-->