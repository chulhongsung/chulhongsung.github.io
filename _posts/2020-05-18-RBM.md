...
볼츠만 머신은 이진 벡터(${\bf x} \in \le{ 0,1 \}^d$)에 대한 확률 분포를 학습하기 위해 제안되었다. 학습된 확률분포를 통해서 새로운 데이터를 생성할 수 있다. 먼저, 확률분포를 모형화해야한다. 통계학에서는 가우시안 분포나 이진 변수이면 베르누이 분포 등을 통해 확률분포를 모형화했을 수 있지만 볼츠만 머신은 에너지 함수라는 물리학 개념을 사용했다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTIyMjQ0MjIsOTM0OTgxOTM5LDE0Mz
