...
더 자세하게 설명해보면,  $p_{emp}$를 현재 데이터(${\bf x}^{(i)}$) 경험 분포($\sum_{i=1}^n \delta_{{\bf x}^{(i)}}$/n)이라 하자. 우리가 갖고 있는 $\bf x$는 $p_{emp}$를 따르고 그 기댓값은 $\fracP{$

그리고 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ2NjMzNzE3MywtMzE0OTE3MjU0XX0=
-->