...
$$
\begin{aligned}
b_j^{(k+1)}= b_j^{(k)} + \frac{\partial \log p_{\theta}({\bf x})}{\partial b_j} &= \frac{1}{n}\sum_{i=1}^n x_j - \sum_{\tilde{\bf x}}p_{\theta}(\tilde{\bf x})x_j, \\
\frac{\partial \log p_{\theta}({\bf x})}{\partial c_k} &= \frac{1}{n}\sum_{i=1}^n p(h_k^{(i)}= 1|{\bf x}^{(i)}) -\sum_{\tilde{\bf x}}p_{\theta}(\tilde{\bf x})p(\tilde{h}_k = 1|{\tilde{\bf x}}), \\
\frac{\partial \log p_{\theta}({\bf x})}{\partial w_{jk}} &= \frac{1}{n}\sum_{i=1}^{n}x_j^{(i)}p(h_k^{(i)}= 1|{\bf x}^{(i)}) - \sum_{\tilde{\bf x}}p_
{\theta}(\tilde{\bf x})\tilde{x}_jp(\tilde{h}_k = 1 | {\tilde{\bf x}}).
\end{aligned}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjU2NjEwODA1LDE2NDgwNDcwLC02NjU1Mj
E1ODUsLTE4MDEwMTA5MjksLTE5OTMzMDMxODIsODcxNjM0NjMw
LC0xMzkyMjYxNjkwLC0xNTY3MzY3MTE1LC0xOTMxOTA5NjgsOT
M0OTgxOTM5LDE0Mzg4ODc3MDMsLTg4OTQ5NTY2NCw2Mzc4ODY3
ODIsMTI3NTM5MDU1OSwtMjU1NTU0OTUxLDE2MzAyNTM0OTQsLT
UyODUwMDE1NiwtMzE0OTE3MjU0XX0=
-->