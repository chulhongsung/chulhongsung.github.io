...
여기서 $\sum_{\bf h}\exp(-E({\bf x}, {\bf h})) = \exp(-F({\bf x}))$이고 $Z = \sum_{\tilde{\bf x}}\exp(-F_{\theta}(\tilde{\bf x}))$이다. $Z$와 같은 정규화항을 무시하고 넘어가는 경우가 많다. 무시해도 무방하기 때문인데 여기서는 그렇지 않다. 자세한 내용은 뒤에서 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTY1NzExNDQ2LDEyNzUzOTA1NTksLTI1NT
