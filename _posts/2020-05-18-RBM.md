...
여기서는 데이터를 $\bf x$로 표현을 했지만 다른 제한된 볼츠만 머신 관련 문헌에서는 관측 가능하다(visible)고 해서 $\bf v$로 표현했다. 
...
위의 그림을 보면 두 에너지 함수의 차이를 쉽게 확인할 수 있을 것이다. 오른쪽의 제한된 볼츠만 머신은 같은 층에 있는 노드들끼리의 연결선이 없다. 
...
제한된 볼츠만 머신의 목적함수는 에너지 함수를 사용해서 정의된 marginal likelihood $p_{\theta}({\bf x})$를 최대화하는 것이다. 즉, 우리가 가지고 있는 데이터(x)의 likelihood를 높이는 것이다. log-likelihood, $\log p_{\theta}({\bf x})$를 모형의 파라미터($\theta$)에 대해서 미분해보자.
...
여기에서 문제가 발생하는데 일반적인 딥러닝이나 머신러닝 모형에서는 우리가 갖고 있는 데이터(${\bf x}^{(i)}$)로 역전파를 이용해서 미분값을 계산할 수 있었다. 하지만, 제한된 볼츠만 머신에서는 미분결과의 뒷항 때문에 우리가 가진 데이터만으로 미분값을 계산하기가 
...
제한된 볼츠만 머신 학습을 위한 샘플링 방법으로는 'Gibbs sampling(깁스 샘플링)'을 사용한다. 깁스 샘플링은 MCMC(Markov Chain Monte Carlo) 방법 중 하나로 매우 간편하고 유용한 샘플링 방법이다. 깁스 샘플링은 쉽게 설명하면 샘플링 해야하는 대상이 m차원이라고 한다면, $\tilde{\bf x} \in \mathbb{R}^m$ 다음과 같이 샘플링을 한다.
...
제한 볼츠만 머신에서는 단순하다. 데이터 $\bf x$ 를 이용해서 $\bf h$를 계산하고, $\bf h$를 가지고 반대로 $\tilde{\bf x}$ 생성한다.  수식으로 정리하면 아래와 같다. 아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자.
...
확률을 계산했으면 0과 1을 가지는 이항변수를 생성하는 것은 균일분포를 통해 쉽게 생성할 수 있다. 

그런 다음 기댓값을 계산해서 파라미터를 업데이트한다. 

위와 같은 방법으로 학습하는 방법을 'Constrastive Divergence Algorithm'이라고 한다. 그리고 깁스 샘플링을 n 번 하면 'n-CD'라고 하는데 1-CD만 하더라도 충분하다는 것이 잘 알려져있다.

다음은 파이썬 코드를 통해 확실하게 알아보도록 하자.

### <span style="color:#F7DC6F;">Python Code</span>


----
이번에 제한된 볼츠만 머신에 대해서 공부해보았는데 제한된 볼츠만 머신도 초창기 모형으로 지금은 많은 발전을 해왔다. 그 중 하나로 Wasserstein Distance를 이용한 제한된 볼츠만 머신도 있다. 그리고 최근에는 제한된 볼츠만 머신을 이용해 임베딩을 하는 기법도 많이 나오고 있으니 딥러닝을 공부하는 사람들에게는 좋은 공부거리가 될 것 같다.
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2MDQyNDc4ODQsODcxNjM0NjMwLC0xMz
kyMjYxNjkwLC0xNTY3MzY3MTE1LC0xOTMxOTA5NjgsOTM0OTgx
OTM5LDE0Mzg4ODc3MDMsLTg4OTQ5NTY2NCw2Mzc4ODY3ODIsMT
I3NTM5MDU1OSwtMjU1NTU0OTUxLDE2MzAyNTM0OTQsLTUyODUw
MDE1NiwtMzE0OTE3MjU0XX0=
-->