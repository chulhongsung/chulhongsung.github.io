...
RBM에서는 단순한데 아래의 식과 같다. 아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk2NjkwOTEzNSw5OTMzMzg3NTAsLTQzMT
YyMDYzMywxNjMzNDE1Mzg5LDg3NzA1MzUyMSwyMTE0ODA5Mzc3
LDE2NDgwNDcwLC02NjU1MjE1ODUsLTE4MDEwMTA5MjksLTE5OT
MzMDMxODIsODcxNjM0NjMwLC0xMzkyMjYxNjkwLC0xNTY3MzY3
MTE1LC0xOTMxOTA5NjgsOTM0OTgxOTM5LDE0Mzg4ODc3MDMsLT
g4OTQ5NTY2NCw2Mzc4ODY3ODIsMTI3NTM5MDU1OSwtMjU1NTU0
OTUxXX0=
-->