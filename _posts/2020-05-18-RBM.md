...
위의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자. 위의 식은 이항변수라는 특징 덕분에 위와 같이 식이 쓰여지는 것을 확인할 수 있다. 확률을 계산했으면 0과 1을 가지는 이항변수를 생성하는 것은 균일분포를 통해 쉽게 생성할 수 있다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5MTM0ODAzMzksLTk2NjkwOTEzNSw5OT
MzMzg3NTAsLTQzMTYyMDYzMywxNjMzNDE1Mzg5LDg3NzA1MzUy
MSwyMTE0ODA5Mzc3LDE2NDgwNDcwLC02NjU1MjE1ODUsLTE4MD
EwMTA5MjksLTE5OTMzMDMxODIsODcxNjM0NjMwLC0xMzkyMjYx
NjkwLC0xNTY3MzY3MTE1LC0xOTMxOTA5NjgsOTM0OTgxOTM5LD
E0Mzg4ODc3MDMsLTg4OTQ5NTY2NCw2Mzc4ODY3ODIsMTI3NTM5
MDU1OV19
-->