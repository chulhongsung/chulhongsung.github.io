...
여기서 $\sum_{\bf h}\exp(-E({\bf x}, {\bf h})) = \exp(-F({\bf x}))$이고 $Z = \sum_{\tilde{\bf x}}\exp(-F_{\theta}(\tilde{\bf x}))$이다. 사실 RBM에서 $Z$를 계산하기가 매우 어렵다. 예를 들어 $\bf h$가 100차원, $\bf x$가 100차원이라고 하고 각각의 원소는 0 또는 1의 값을 가지므로 $Z$를 계산하기 위해서 $2^{100} \times 2^{100}$의 경우의 수에 대해서 합을 계산해야한다. 바로 이러한 문제가 발생해서 이것을 해결하려는 노력이 있었다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTg5MTQ2ODY3LC0xODAxMDEwOTI5LC0xOT
kzMzAzMTgyLDg3MTYzNDYzMCwtMTM5MjI2MTY5MCwtMTU2NzM2
NzExNSwtMTkzMTkwOTY4LDkzNDk4MTkzOSwxNDM4ODg3NzAzLC
04ODk0OTU2NjQsNjM3ODg2NzgyLDEyNzUzOTA1NTksLTI1NTU1
NDk1MSwxNjMwMjUzNDk0LC01Mjg1MDAxNTYsLTMxNDkxNzI1NF
19
-->