...
볼츠만 머신은 이진 벡터(${\bf x} \in \left\{ 0,1 \right\}^d$)에 대한 확률 분포를 학습하기 위해 제안되었다. 학습된 확률분포를 통해서 새로운 데이터를 생성할 수 있다. 먼저, 확률분포를 모형화해야한다. 통계학에서는 가우시안 분포나 이진 변수이면 베르누이 분포 등을 통해 확률분포를 모형화했을 수 있지만 볼츠만 머신은 에너지 함수라는 물리학 개념을 사용했다.
...
$F_{\theta}({\bf x})$는 $\bf h$에 대한 marginal distribution이므로 $\bf h_k \in \left\{0 ,1\right\}$임을 이용하면 $F_{\theta}({\bf x})$는 다음과 같이 쓸 수 있다.
...
$$
\tilde{x}_j^{(k)} \sim p(\tilde{x}_j|\tilde{x}_1, \dots, \tilde{x}_{j-1}, \tilde{x}_{j+1}, \dots , \tilde{x}_m)
$$ 

나머지 $x_i, i\neq j$를 고정시키고 $x_j$를 뽑는다. 

하지만 RBM에서는 굳이 위와 같은 방식으로 샘플링을 할 필요가 없다. 데이터($\bf x$)를 이용해서 ${\bf c} + {\bf W}$




<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNTk2MzY1MjEsLTE5MzE5MDk2OCw5Mz
Q5ODE5MzksMTQzODg4NzcwMywtODg5NDk1NjY0LDYzNzg4Njc4
MiwxMjc1MzkwNTU5LC0yNTU1NTQ5NTEsMTYzMDI1MzQ5NCwtNT
I4NTAwMTU2LC0zMTQ5MTcyNTRdfQ==
-->