...
RBM에서는 단순하다. 데이터 ${\bf x}$ 를 이용해서 ${\bf h}$를 계산하고, ${\bf h}$를 가지고 반대로 $\tilde{\bf x}$를 생성한다.  수식으로 정리하면 아래와 같다. 아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYzMzQxNTM4OSw4NzcwNTM1MjEsMjExND
gwOTM3NywxNjQ4MDQ3MCwtNjY1NTIxNTg1LC0xODAxMDEwOTI5
LC0xOTkzMzAzMTgyLDg3MTYzNDYzMCwtMTM5MjI2MTY5MCwtMT
U2NzM2NzExNSwtMTkzMTkwOTY4LDkzNDk4MTkzOSwxNDM4ODg3
NzAzLC04ODk0OTU2NjQsNjM3ODg2NzgyLDEyNzUzOTA1NTksLT
I1NTU1NDk1MSwxNjMwMjUzNDk0LC01Mjg1MDAxNTYsLTMxNDkx
NzI1NF19
-->