...
RBM 학습을 위한 샘플링 방법으로는 'Gibbs sampling(깁스 샘플링)'을 사용한다. 깁스 샘플링은 MCMC 방법 중 하나로 매우 간편하고 유용한 샘플링 방법이다. 깁스 샘플링은 쉽게 설명하면 샘플링 해야하는 대상이 m차원이라고 한다면 즉$\tilde{\bf x} \in \mathbb{R}^m$ 

RBM에서 왜 깁스 샘플링을 사용하는가 하면 


<!--stackedit_data:
eyJoaXN0b3J5IjpbMTgzNDgzNDQ3NiwtODg5NDk1NjY0LDYzNz
