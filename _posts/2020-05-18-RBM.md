...
더 자세하게 설명해보면,  $p_{emp}$를 현재 데이터(${\bf x}^{(i)}$) 경험 분포($\sum_{i=1}^n \delta_{{\bf x}^{(i)}}$/n)이라 하자. 우리가 갖고 있는 $\bf x$는 $p_{emp}$를 따르고 그 기댓값은 $\frac{1}{n}\sum_{i=1}^nf(\cdot)$의 형태로 계산되어진다. 뒷항은 $\bf x$을 

그리고 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTcwNzk3NDE2NiwtMzE0OTE3MjU0XX0=
-->