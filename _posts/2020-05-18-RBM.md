...
여기서 $\sum_{\bf h}\exp(-E({\bf x}, {\bf h})) = \exp(-F({\bf x}))$이고 $Z = \sum_{\tilde{\bf x}}\exp(-F_{\theta}(\tilde{\bf x}))$이다. 사실 RBM에서 $Z$를 계산하기가 매우 어렵다. 예를 들어 $\bf h$가 10차원, $\bf x$가 10차원이라고 하면 $2^10 \$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyNzE5OTU3MDksLTE4MDEwMTA5MjksLT
