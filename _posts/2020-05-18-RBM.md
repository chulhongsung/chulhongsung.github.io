...
RBM에서는 단순하다. 데이터 $\bf x$ 를 이용해서 $\bf h$를 계산하고, $\bf h$를 가지고 반대로 $\tilde{\bf x}$ 생성한다.  수식으로 정리하면 아래와 같다. 아래의 식에서 $p(\bf h = 1 |x)$와 $p(\tilde{\bf h = 1 |x)$의 의미는 $\bf h$의 원소들이 1인 확률을 모아놓은 벡터를 의미한다.
$$
\begin{aligned}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjMyMjA5NTYxLDg3MTYzNDYzMCwtMTM5Mj
