...
갑자기 샘플링이 나와서 당황할 수 있다. 사실 머신러닝의 입문서에서 다루는 모형들은 대부분 샘플링을 필요로 하지 않는다. 말그대로 주어진 데이터에 기반해서 파라미터를 추정하면 그만이다. 하지만 입문을 벗어나 확률분포를 추론하는 모형과 같은 심화 모형들은 샘플링 과정을 거치게 된다. 예를 들면 "VAE"가 있다. VAE는 입문서에서 이론을 다루기에는 통계학적인 베이스가 상당히 요구된다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYxMTczMjQzNiwtMjU1NTU0OTUxLDE2Mz
