...
RBM에서는 단순하다. 데이터 $\bf x$ 를 이용해서 $\bf h$를 계산하고, $\bf h$를 가지고 반대로 $\tilde{\bf x}$ 생성한다.  수식으로 정리하면 아래와 같다. 아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 ${\bf h}^{(n)}$과 $\$의 원소들이 1인 확률을 모아놓은 벡터를 의미한다.
$$
\begin{aligned}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTcxNjYwNTM4OSw4NzE2MzQ2MzAsLTEzOT
IyNjE2OTAsLTE1NjczNjcxMTUsLTE5MzE5MDk2OCw5MzQ5ODE5
MzksMTQzODg4NzcwMywtODg5NDk1NjY0LDYzNzg4Njc4MiwxMj
c1MzkwNTU5LC0yNTU1NTQ5NTEsMTYzMDI1MzQ5NCwtNTI4NTAw
MTU2LC0zMTQ5MTcyNTRdfQ==
-->