...
$$
\begin{aligned}
p({h}_j^{(n)} = 1  | {\bf x}^{(n)}) &= sigmoid\left(({\bf W}{\bf x}^{(n)} + {\bf c})_j\right)\\
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNTQ2MzkxNDgsLTQ1ODAzOTExNCwtMTkxMz
Q4MDMzOSwtOTY2OTA5MTM1LDk5MzMzODc1MCwtNDMxNjIwNjMz
LDE2MzM0MTUzODksODc3MDUzNTIxLDIxMTQ4MDkzNzcsMTY0OD
