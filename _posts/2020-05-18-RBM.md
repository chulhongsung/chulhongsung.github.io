...
$$
\begin{aligned}
p({h}_j^{(n)} = 1  | {\bf x}^{(n)}) &= sigmoid\left(\left({\bf W}{\bf x}^{(n)} + {\bf c}\right)_j)\\
p(\tilde{ x}_j^{(n)} = 1 |  {\bf h}^{(n)}) &= sigmoid(\left({\bf W}^\top  {\bf h}^{(n)} + {\bf b}\right)_j)\\
\end{aligned}
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ5OTU4NTY3NywtNDU4MDM5MTE0LC0xOT
EzNDgwMzM5LC05NjY5MDkxMzUsOTkzMzM4NzUwLC00MzE2MjA2
MzMsMTYzMzQxNTM4OSw4NzcwNTM1MjEsMjExNDgwOTM3NywxNj
