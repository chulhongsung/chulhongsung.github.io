...
$F_{\theta}({\bf x})$는 $\bf h$에 대한 marginal distribution이므로 $\bf h \in \left\{0 ,1\right\}^k$임을 이용하면 $F_{\theta}({\bf x})$는 다음과 같이 쓸 수 있다.
...
하지만 RBM에서는 굳이 위와 같은 방식으로 샘플링을 할 필요가 없다. 데이터($\bf x$)를 이용해서 $$




<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUwNzYyNjk5MCw5MzQ5ODE5MzksMTQzOD
