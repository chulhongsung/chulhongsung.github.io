...
여기서 $\sum_{\bf h}\exp(-E({\bf x}, {\bf h})) = \exp(-F({\bf x}))$이고 $Z = \sum_{\tilde{\bf x}}\exp(-F_{\theta}(\tilde{\bf x}))$이다. 사실 RBM에서 $Z$를 계산하기가 매우 어렵다. 예를 들어 $\bf h$가 100차원, $\bf x$가 100차원이라고 하고 각각의 원소는 0 또는 1의 값을 가지므로 $Z$를 계산하기 위해서 $2^{100} \times 2^{100}$의 경우의 수에 대해서 합을 계산해야한다. 하지만 모델의 ㅇ 우리가 필요로 하는 것은 미분계수이다. 미분 수식을 예쁘게 전개하면 $Z$를 계산하지 않고도 미분 결과를 알 수 있다.
...
위에서 언급된 $Z$ 때문에 뒤에 익숙한 항이 하나 생겼다. 바로 $F_\theta$의 미분값의 기댓값이다. 모형을 학습하기 위해서는 미분 결과만 알면 된다. 굳이 $Z$를 계산할 필요가 없는 것이다. 여기서 log-likelihood를 n으로 나누더라도 최대화하는 방향은 똑같으므로 다음의 식을 사용하도록 한다.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNjU5MDkzNDMsLTE4MDEwMTA5MjksLT
E5OTMzMDMxODIsODcxNjM0NjMwLC0xMzkyMjYxNjkwLC0xNTY3
MzY3MTE1LC0xOTMxOTA5NjgsOTM0OTgxOTM5LDE0Mzg4ODc3MD
MsLTg4OTQ5NTY2NCw2Mzc4ODY3ODIsMTI3NTM5MDU1OSwtMjU1
NTU0OTUxLDE2MzAyNTM0OTQsLTUyODUwMDE1NiwtMzE0OTE3Mj
U0XX0=
-->