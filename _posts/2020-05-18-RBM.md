...
RBM에서는 단순하다. 데이터 $\bf x$아래의 식에서 $p({\bf h}^{(n)} = 1 |{\bf x}^{(n)})$와 $p(\tilde{\bf x}^{(n)} = 1 |{\bf h}^{(n)})$의 의미는 각각 ${\bf h}^{(n)}$과 $\tilde{\bf x}^{(n)}$의 원소들이 1이 될 확률를 원소로 가지는 벡터를 의미한다고 생각하자. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTg4OTgxODI4NywtMTkxMzQ4MDMzOSwtOT
Y2OTA5MTM1LDk5MzMzODc1MCwtNDMxNjIwNjMzLDE2MzM0MTUz
ODksODc3MDUzNTIxLDIxMTQ4MDkzNzcsMTY0ODA0NzAsLTY2NT
