...
$\mu$와 $\Sigma$의 추정치로 표본평균과 표본공분산 행렬을 사용한다. $\hat{\mu} = \frac{1}{T}\sum_{t=1}^{T}r_t$와 $\hat{\Sigma} =(R-\bold{1}_T\hat{\mu}^{\top})^{}$ 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE0MDE3MjQwNSwtMTEzMjYyMzYxNiwtMz
