...
여기서 $\beta = \Omega^{-1}\mu = E(R^{\top}R)E(R^{\top}1_{T})$.

$\mu$와 $\Sigma$의 추정치로 표본평균과 표본공분산 행렬을 사용한다. $\hat{\mu} = \frac{1}{T}\sum_{t=1}^{T}r_t$와 $\hat{\Sigma} =(R-1_T\hat{\mu}^{\top})^{\top}(R-1_T\hat{\mu}^{\top})/T$으로 $x^{*}$의 추정치 $\hat{x}=\hat{\Sigma}^{-1}\hat{\mu}/\gamma$ 구할 수 있다. 


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIxMzMzNjI1OTcsLTE0OTMxODY4MTcsLT
ExMzI2MjM2MTYsLTM3ODk2NTI3MCwxODEyNDk2NTUyLDU3NzA0
MjIxMiwtMjA0MTgxMzYxNywtMTUxODgzMTg1NywtMTAwMTk2ND
Y3NSwyMTIyNDg4MTI3LDExMDA4MzExMTAsLTE2NzMyMTE5NTAs
MTk5MTEwNjE4Niw3MzY3NDE4NDUsMTAwNTg1NDM1NywxNzA4NT
g3MDgxLDczNzA2MjI3LC0zNzYyNDI0NDUsMTM2NjY0MDgzNywt
MTY1Mjk3OTc2MF19
-->