...
여기서 $\beta = \Omega^{-1}\mu = E(R^{\top}R)E(R^{\top}\bold{1}_{T})$.

$\mu$와 $\Sigma$의 추정치로 표본평균과 표본공분산 행렬을 사용한다. $\hat{\mu} = \frac{1}{T}\sum_{t=1}^{T}r_t$와 $\hat{\Sigma} =(R-\bold{1}_T\hat{\mu}^{\top})^{\top}(R-\bold{1}_T\hat{\mu}^{\top})/T$으로 $x^{*}$의 추정치 $\hat{x}=\hat{\Sigma}^{-1}\hat{\mu}/\gamma$ 구할 수 있다. 
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwNTEyMzU0MCwtMTEzMjYyMzYxNiwtMz
