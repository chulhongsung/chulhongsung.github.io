...
$$
\underbrace{(p^\star - p^{k+1}) + (p^{k+1} - p^\star)}_{=0} \leq \underbrace{(y^\star - y^{k+1})^\top r^{k+1}}_{(1)} + \rho(B(z^{k+1} - z^k))^\top r^{k+1}- \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^\star)
$$
...
$$
(1/\rho)\left( \|  y^{k+1} - y^\star \|_2^2  - \| y^k - y^\star\|_2^2 \right) + \underbrace{\rho \| r^{k+1}\|_2^2}_{(2)}. \tag{A.5}
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ5ODA1Mjc3MywxMjA4MTkwOTQ1LDEzOT
