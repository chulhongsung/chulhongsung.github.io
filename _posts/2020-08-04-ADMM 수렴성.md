...
A.3, A.2 두 부등식을 더해보자.
$$
p^{k+1} + y^{k+1}(Ax^{k+1} + Bz^{k+1}) - \rho (z^{k+1} -z^k)^\top B^\top Z x^{k+1} \leq \\ p^\star + y^{k+1}(Ax^\star + Bz^\star) - \rho (z^{k+1} -z^k)^\top B^\top Z x^\star 
$$

위의 식을 잘 정리하면 다음의 결과를 얻는다. 
$$
p^{k+1} - p^\star \leq -(y^{k+1})^\top - \rho (B(z^{k+1} - z^k))^\top(-r^{k+1} + B(z^{k+1} - z^\star)) \tag{A.2}
$$
(1) 부등식과 A.3 부등식을 이용해보자.
$$
p^\star - p^{k+1} \leq   y^{\star\top}r^{k+1}  \tag{A.3}
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTIwODgyMTg1LDU4NzkyNTI5MSwxMzc5Mz
YyMTMzLDExNTI2MzE3NzEsLTE3NjMxMzE0MjFdfQ==
-->