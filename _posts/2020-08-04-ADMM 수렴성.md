...
위의 부등식의 결과, 무한합이 bounded되며 더해지는 항은 $0$ 이상이므로 수렴해야한다.  $\left( \|r^{k+1}\|_2^2 + \|B(z^{k+1} - z^k)\|_2^2 \right) \rightarrow 0$ as $k \rightarrow 0$이어야 한다. 즉,  $r^k\rightarrow 0$ 그리고 $B(z^{k+1} - z^k) \rightarrow 0$이다. 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjEyMTg3NjQxNSw5ODM4NjI2NTYsMTc5OT
