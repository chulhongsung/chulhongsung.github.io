...
위의 (A.1)식에 사용된 $V^k = (1/\rho)\| y^k - y^\star\|_2^2 +\rho \| B(z^k - z^\star)\|_2^2$을 자세히 보자. 우선 $V^k \geq 0$이고 $V^k$는 감소함을 알 수 있다.  그리고 (A.1)을 $k=0, \dots, k$ 인 경우를 모두 더하면 

$$
\begin{aligned}
V^0 - V^1 &\geq \rho \| r^1\|_2^2 + \rho \|B(z^{k+1} - z^k)\|_2^2 \\
V^0 - V^1 &\geq \rho \| r^1\|_2^2 + \rho \|B(z^{k+1} - z^k)\|_2^2 \\
&~~~~ \vdots\\
V^0 - V^1 &\geq \rho \| r^1\|_2^2 + \rho \|B(z^{k+1} - z^k)\|_2^2 
\end{aligned}
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA3ODkxMTkwNiwtMTE4MzczMzcyLC0xNT
