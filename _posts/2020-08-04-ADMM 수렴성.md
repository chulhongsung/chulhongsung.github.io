...
$$
\underbrace{(p^\star - p^{k+1}) + (p^{k+1} - p^\star)}_{=0} \leq \underbrace{(y^\star - y^{k+1})^\top r^{k+1}}_{=~\text{first term}} + \rho(B(z^{k+1} - z^k))^\top r^{k+1} - \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^\star)
$$
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NDUxNjQ0NzksMTMwOTY3OTAwNyw5Mj
A4ODIxODUsNTg3OTI1MjkxLDEzNzkzNjIxMzMsMTE1MjYzMTc3
MSwtMTc2MzEzMTQyMV19
-->