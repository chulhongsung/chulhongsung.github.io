...
### <span style="color:#F7DC6F;"> A.3 	증명</span>
-----
...
### <span style="color:#F7DC6F;"> A.2	증명</span>
-----
...
### <span style="color:#F7DC6F;"> A.1	증명</span>
-----
...
$$
0 \geq \underbrace{2 (y^{k+1} - y^\star)^\top r^{k+1}}_{(1)} \underbrace{ -2\rho(B(z^{k+1} - z^k))^\top r^{k+1} +2 \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^\star)}_{(2)}
$$
...
$$
(1/\rho)\left( \|  y^{k+1} - y^\star \|_2^2  - \| y^k - y^\star\|_2^2 \right) + \underbrace{\rho \| r^{k+1}\|_2^2}_{(3)}. \tag{A.5}
$$
...
$$
\begin{aligned}
&\rho \| r^{k+1}\|_2^2 -2\rho(B(z^{k+1} - z^k))^\top r^{k+1} + \rho(B(z^{k+1} - z^k))^\top \left(B(z^{k+1} - z^k ) +B(z^k - z^\star) \right)\\
=&\rho \| r^{k+1}\|_2^2 -2\rho(B(z^{k+1} - z^k))^\top r^{k+1} + \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^k ) +B(z^k - z^\star)
\end{aligned}
$$

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQ5MDkxNDE4NywxMjA4MTkwOTQ1LDEzOT
Y0NzU1NTEsLTE2NDUxNjQ0NzksMTMwOTY3OTAwNyw5MjA4ODIx
ODUsNTg3OTI1MjkxLDEzNzkzNjIxMzMsMTE1MjYzMTc3MSwtMT
c2MzEzMTQyMV19
-->