...
$$
p^{k+1} - p^\star \leq -(y^{k+1})^\top - \rho (B(z^{k+1} - z^k))^\top(-r^{k+1} + B(z^{k+1} - z^\star))
$$
위에서 증명한 A.3 부등식을 이용해보자.
$$
p^\star - p^{k+1} \leq  + y^{\star\top}r^{k+1}  \tag{A.3}
$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwOTUxNTA2ODEsMTM3OTM2MjEzMywxMT
UyNjMxNzcxLC0xNzYzMTMxNDIxXX0=
-->