...
ADMM 알고리즘 과정 중 $x^{k+1}$은 $L_\rho(x, z^k, y^k)$를 최소화한다. 따라서, optimal condition은 다음과 같다.
$$
0 \in \partial L_\rho(x^{k+1}, z^k, y^k) = \partial f(x^{k+1}) + A^\top y^k + \rho A^\top (A^{k+1}) 
$$
...
$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA1MDcyMDQyMSwxMTUyNjMxNzcxLC0xNz
YzMTMxNDIxXX0=
-->