...
=&\rho\|r^{k+1} - B(z^{k+1} - z^k))\|_2^2+ \rho\|(B(z^{k+1} - z^k))\|_2^2 + 2\rho(B\underbrace{(z^{k+1} - z^k)}_{(z^{k+1} - z^{\star}) - (z^k -z)})^\top B(z^k - z^\star)
\end{aligned}
$$

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE1OTQxMDAzMDksMTIwODE5MDk0NSwxMz
