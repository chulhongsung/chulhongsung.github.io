...
\end{aligned}
$$
위의 결과는 $x^{k+1}$이 $f(x) + (y^{k+1} - \rho B(z^{k+1} - z^k))^\top Ax$의 최소자(minimizer)임을 의미한다. 똑같은 과정을 $z$에 대해서도 하면 $z^{k+1}$은 $g(z) + y^{k+1\top}Bz$의 최소자이다.
...
$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTkzNjYxNDI0OCwxMzc5MzYyMTMzLDExNT
