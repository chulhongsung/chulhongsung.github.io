...
$$
\underbrace{(p^\star - p^{k+1}) + (p^{k+1} - p^\star)}_{=0} \leq \underbrace{(y^\star - y^{k+1})^\top r^{k+1}}_{(1)} + \underbrace{\rho(B(z^{k+1} - z^k))^\top r^{k+1}}_{(2)}- \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^\star)
$$

$$
0 \geq 2 (y^{k+1} - y^\star)^\top r^{k+1}  -2\rho(B(z^{k+1} - z^k))^\top r^{k+1} +2 \rho(B(z^{k+1} - z^k))^\top B(z^{k+1} - z^\star)
$$
우변의 첫번째 항(위 식에서 first term)에서 $y^{k+1}$ 대신에 $y^{k+1} = y^k + \rho r^{k+1}$을 대입하면
$$
2(y^k + \rho r^{k+1} -y^\star)^\top r^{k+1} = 2(y^k - y^\star)^\top r^{k+1}
$$

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3MjY4NTA3NzIsMTM5NjQ3NTU1MSwtMT
Y0NTE2NDQ3OSwxMzA5Njc5MDA3LDkyMDg4MjE4NSw1ODc5MjUy
OTEsMTM3OTM2MjEzMywxMTUyNjMxNzcxLC0xNzYzMTMxNDIxXX
0=
-->