...
$$
\underbrace{(1/\rho)\left( \|  y^{k+1} - y^\star \|_2^2  - \| y^k - y^\star\|_2^2 \right)}_{(*)} + \underbrace{\rho \| r^{k+1}\|_2^2}_{(3)}.
$$
...
(A.1)의 부등식을 만족하는 것은 (A.6)의 우변의 전개식에서 가운데 $-2\rho((r^{k+1})^\top B(z^{k+1} - z^k))$가 양수임과 동치이다. 그리고 

$$

$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExMDEzNTQwMDksMTIwODE5MDk0NSwxMz
